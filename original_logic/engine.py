...
def new_connect_four():
    ...
    return {"board": board, "turn": 1, "winner": 0, "engine": "original-python"}


def play_connect_four(board, turn, column):
    if connect_four.winner(board) or not connect_four.drop(board, 2, column):
        return {"board": board, "turn": turn, "winner": connect_four.winner(board), "engine": "original-python"}
    turn += 1
    ...
    if not won:
        ...
        turn += 1
        won = connect_four.winner(board)
    return {"board": board, "turn": turn, "winner": won, "engine": "original-python"}
...
def new_tic_tac_toe():
    ...
    return {"board": board, "winner": None, "engine": "original-python"}


def play_tic_tac_toe(board, index):
    row, column = divmod(index, 3)
    if board[row][column] != "N" or tic_tac_toe.checkWin(board)[0]:
        return {"board": board, "winner": tic_tac_toe.checkWin(board)[1], "engine": "original-python"}
    board[row][column] = "O"
    ...
    return {"board": board, "winner": winner, "engine": "original-python"}
